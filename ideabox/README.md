...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv ideabox` or `python -m venv ideabox-venv`
...
- [ ] Make sure the virtualenv is activated `workon ideabox` or `source ideabox-venv/bin/activate`
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `ideabox/backend/ideabox/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `ideabox/backend/ideabox/settings/production.py`
...
- Create a copy of ``ideabox/settings/local.py.example``:  
  `cp ideabox/settings/local.py.example ideabox/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/ideabox/settings/base.py``.
- Adjust ``backend/ideabox/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
